...
def read_test_set(df_test, dictionary, SEQUENCE_LEN_D = 40, SEQUENCE_LEN = 65, BATCH_SIZE = 10):
    ...
    for i in df_test['text']:
        i = sent_tokenize(i)
        X_test_.append([dictionary['START_SENT']])
        for j in i[:SEQUENCE_LEN_D-2]:
            ...
            X_test_.append(data)
        X_test_.append([dictionary['END_SENT']])
        for k in range(max(SEQUENCE_LEN_D -  (len(i)+2), 0)):
            X_test_.append([0]) # pad token maps to 0

    print('len of test set: ', len(X_test_)//BATCH_SIZE)
    print('IV in test set: ', count_iv_train)
    print('OOV in test set: ', count_oov_train)
